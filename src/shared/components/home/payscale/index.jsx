...
// Country data generators for each tech stack
...
export default function TechStackDashboard() {
  ...
  // Memoize the current content to avoid recalculating
  const currentContent = useMemo(() => {
    const item = SIDEBAR_ITEMS[activeSidebarItem];
    return item.content(item.tabs[activeTab]);
  ...
  const tabStyles = useMemo(() => ({
    ...
    '& .Mui-selected': {
      background: 'rgba(54, 63, 70, 0.5)',
      color: 'white !important',
    },
    '& .MuiTabs-indicator': {
      backgroundColor: '#e92e3e',
    },
    backgroundColor: 'rgba(54, 63, 70, 0.1)',
          ...
          <Paper elevation={1} className="p-0 h-100">
            ...
            <Box className="mb-3">
              <Tabs
                ...
                sx={tabStyles}
              >
                {SIDEBAR_ITEMS[activeSidebarItem].tabs.map((tab, index) => (
                  <Tab
                    key={index}
                    label={tab}
                    id={`tab-${index}`}
