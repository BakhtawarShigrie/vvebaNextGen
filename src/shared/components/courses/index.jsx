...
const CourseSelector = () => {
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      >
        ...
        >
          {selectedCourse.navigation.map((nav) => (
            ...
            <Button
              className="ripple-button"   // add class
              key={nav.id}
            ...
            >
              <RippleButtonEffect />   {/* add  Ripple Effect     */}     
              {nav.title}
            </Button>
          ))}
        </Box>
      ...
      <Paper elevation={3} sx={{ p: 3, height: 400, mt: 2, overflow: 'auto', scrollbarWidth: 'thin' }}>
        {(() => {
          switch (selectedNav) {
            case 'detail':
              return (
                  ...
                  {selectedCourse.navigation
                    ...
                    ?.content?.map((item, index) => (
                      <Box key={index} sx={{ mb: 2 }}>
                        ...
                        >
                          <Image src={item.imageLink} width={200} height={200} alt={item.title} />
                        ...
                        >
                          <Link href={item.link} target="_blank" style={{ color: 'white', textDecoration: 'none' }}>
                          ...
                          <Grid item key={index}>
                            <Button
                              ...
                              sx={{
                                ...
                                background: selectedTool?.title === item.title || (index === 0 && !selectedTool) ? selectedColor : 'transparent',
                                color: selectedTool?.title === item.title || (index === 0 && !selectedTool) ? "white" : 'black'
                              }}
                            >
                              {item.title}
                            </Button>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'center', mt: 4 }}>
                      <Card sx={{ maxWidth: 1345 }}>
                        <CardMedia
                          component="img"
                          height="300"
                          image={selectedTool?.imageLink || selectedCourse.navigation
                            .find((nav) => nav.id === 'authentication')
                            ?.content?.[0]?.imageLink}
                          alt={selectedTool?.title || selectedCourse.navigation
                            .find((nav) => nav.id === 'authentication')
                            ?.content?.[0]?.title}
                        />
                      </Card>
